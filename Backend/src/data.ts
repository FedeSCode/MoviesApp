export const sample_movies: any[] = [
  {
    id: "0",
    title: "Captain America : First Avenger",
    director: [{ photo: "", name: "Joe Johnston" }],
    screenwriters: [
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/f/f2/Christopher_Markus_%26_Stephen_McFeely_%2848385355446%29.jpg/1920px-Christopher_Markus_%26_Stephen_McFeely_%2848385355446%29.jpg",
        name: "Christopher Markus",
      },
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/f/f2/Christopher_Markus_%26_Stephen_McFeely_%2848385355446%29.jpg/1920px-Christopher_Markus_%26_Stephen_McFeely_%2848385355446%29.jpg",
        name: "Stephen McFeely",
      },
    ],
    actors: [
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/3/33/Mark_Kassen%2C_Tony_C%C3%A1rdenas_and_Chris_Evans_%28cropped%29.jpg",
        name: "Chris Evans",
        role: "Captain America",
      },
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a5/HVFFLondon2017Cap-ALS-29_%2835183142201%29_Cropped.jpg/800px-HVFFLondon2017Cap-ALS-29_%2835183142201%29_Cropped.jpg",
        name: "Hayley Atwell",
        role: "Peggy Carter",
      },
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/e/ee/Sebastian_Stan_by_Gage_Skidmore_2_%28cropped%29.jpg/800px-Sebastian_Stan_by_Gage_Skidmore_2_%28cropped%29.jpg",
        name: "Sebastian Stan",
        role: "James Barnes (Bucky)",
      },
    ],
    plot: "Captain America: First Avenger nous plonge dans les premières années de l’univers Marvel. Steve Rogers, frêle et timide, se porte volontaire pour participer à un programme expérimental qui va le transformer en un Super Soldat connu sous le nom de Captain America. Allié à Bucky Barnes et Peggy Carter, il sera confronté à la diabolique organisation HYDRA dirigée par le redoutable Red Skull.",
    poster:
      "https://upload.wikimedia.org/wikipedia/en/3/37/Captain_America_The_First_Avenger_poster.jpg",
    year: "2011",
    trailer: "IsiV9IJieMk",
    reviews: "GOOD",
    streaming: [{ name: "Disney", plataform: "https://www.disneyplus.com" }],
    stars: 4,
    favorite: false,
  },
  {
    id: "1",
    title: "Iron Man",
    director: [
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/e/eb/Jon_Favreau_2016.jpeg",
        name: "Jon Favreau",
      },
    ],

    screenwriters: [
      {
        photo:
          "https://m.media-amazon.com/images/M/MV5BMTk1ODk2OTM0OF5BMl5BanBnXkFtZTcwNDE3NDA3MQ@@._V1_UY1200_CR115,0,630,1200_AL_.jpg",
        name: "Mark Fergus",
      },
      {
        photo:
          "https://static.cinemagia.ro/img/db/actor/02/34/34/hawk-ostby-628646l.jpg",
        name: "Hawk Ostby",
      },
    ],
    actors: [
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/9/94/Robert_Downey_Jr_2014_Comic_Con_%28cropped%29.jpg",
        name: "Robert Downey Jr.",
        role: "Tony Stark",
      },
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/e/eb/Jon_Favreau_2016.jpeg",
        name: "Jon Favreau",
        role: "Happy Hogan",
      },
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/e/eb/Gwyneth_Paltrow_avp_Iron_Man_3_Paris_2.jpg",
        name: "Gwyneth Paltrow",
        role: "pepper potts",
      },
    ],
    plot: "Après avoir été retenu prisonnier dans une grotte afghane, l'ingénieur milliardaire Tony Stark crée une armure unique pour lutter contre le mal.",
    poster:
      "https://www.themoviedb.org/t/p/original/egEkFOCl87aHJLkIGXGxzNdpQV3.jpg",
    year: "2008",
    trailer: "rDCTb9Gp2qk",
    reviews: "",
    streaming: [
      { name: "PrimeVideo", plataform: "https://www.primevideo.com" },
      { name: "Disney", plataform: "https://www.disneyplus.com" },
    ],
    stars: 5,
    favorite: true,
  },
  {
    id: "2",
    title: "Marvel's The Avengers",
    director: [
      {
        photo:
          "https://fr.wikipedia.org/wiki/Joss_Whedon#/media/Fichier:Joss_Whedon_by_Gage_Skidmore_8.jpg",
        name: "	Joss Whedon",
      },
    ],

    screenwriters: [
      {
        photo:
          "https://m.media-amazon.com/images/M/MV5BMTk1ODk2OTM0OF5BMl5BanBnXkFtZTcwNDE3NDA3MQ@@._V1_UY1200_CR115,0,630,1200_AL_.jpg",
        name: "Mark Fergus",
      },
      {
        photo:
          "https://static.cinemagia.ro/img/db/actor/02/34/34/hawk-ostby-628646l.jpg",
        name: "Hawk Ostby",
      },
    ],
    actors: [
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/9/94/Robert_Downey_Jr_2014_Comic_Con_%28cropped%29.jpg",
        name: "Robert Downey Jr.",
        role: "Tony Stark",
      },
      {
        photo:
          "https://fr.wikipedia.org/wiki/Chris_Evans_(acteur)#/media/Fichier:Mark_Kassen,_Tony_C%C3%A1rdenas_and_Chris_Evans_(cropped).jpg",
        name: "Chris Evans",
        role: "Capitain America",
      },
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/e/eb/Gwyneth_Paltrow_avp_Iron_Man_3_Paris_2.jpg",
        name: "Gwyneth Paltrow",
        role: "pepper potts",
      },
    ],
    plot: "Les plus grands héros de la Terre doivent s'unir et apprendre à se battre s'ils veulent empêcher Loki et son armée extraterrestre d'asservir l'humanité tout entière.",
    poster:
      "https://images.now-tv.com/shares/vod_images/vi_vodproduct_desc_t/201406/en_us/original_vertical/201406040092349_408_608",
    year: "2008",
    trailer: "tWCwPirJSck",
    reviews: "",
    streaming: [
      { name: "PrimeVideo", plataform: "https://www.primevideo.com" },
      { name: "Disney", plataform: "https://www.disneyplus.com" },
    ],
    stars: 1,
    favorite: true,
  },
  {
    id: "2",
    title: "Marvel's The Avengers",
    director: [
      {
        photo:
          "https://fr.wikipedia.org/wiki/Joss_Whedon#/media/Fichier:Joss_Whedon_by_Gage_Skidmore_8.jpg",
        name: "	Joss Whedon",
      },
    ],

    screenwriters: [
      {
        photo:
          "https://m.media-amazon.com/images/M/MV5BMTk1ODk2OTM0OF5BMl5BanBnXkFtZTcwNDE3NDA3MQ@@._V1_UY1200_CR115,0,630,1200_AL_.jpg",
        name: "Mark Fergus",
      },
      {
        photo:
          "https://static.cinemagia.ro/img/db/actor/02/34/34/hawk-ostby-628646l.jpg",
        name: "Hawk Ostby",
      },
    ],
    actors: [
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/9/94/Robert_Downey_Jr_2014_Comic_Con_%28cropped%29.jpg",
        name: "Robert Downey Jr.",
        role: "Tony Stark",
      },
      {
        photo:
          "https://fr.wikipedia.org/wiki/Chris_Evans_(acteur)#/media/Fichier:Mark_Kassen,_Tony_C%C3%A1rdenas_and_Chris_Evans_(cropped).jpg",
        name: "Chris Evans",
        role: "Capitain America",
      },
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/e/eb/Gwyneth_Paltrow_avp_Iron_Man_3_Paris_2.jpg",
        name: "Gwyneth Paltrow",
        role: "pepper potts",
      },
    ],
    plot: "Les plus grands héros de la Terre doivent s'unir et apprendre à se battre s'ils veulent empêcher Loki et son armée extraterrestre d'asservir l'humanité tout entière.",
    poster:
      "https://images.now-tv.com/shares/vod_images/vi_vodproduct_desc_t/201406/en_us/original_vertical/201406040092349_408_608",
    year: "2008",
    trailer: "tWCwPirJSck",
    reviews: "",
    streaming: [
      { name: "PrimeVideo", plataform: "https://www.primevideo.com" },
      { name: "Disney", plataform: "https://www.disneyplus.com" },
    ],
    stars: 0,
    favorite: true,
  },
  {
    id: "2",
    title: "Marvel's The Avengers",
    director: [
      {
        photo:
          "https://fr.wikipedia.org/wiki/Joss_Whedon#/media/Fichier:Joss_Whedon_by_Gage_Skidmore_8.jpg",
        name: "	Joss Whedon",
      },
    ],

    screenwriters: [
      {
        photo:
          "https://m.media-amazon.com/images/M/MV5BMTk1ODk2OTM0OF5BMl5BanBnXkFtZTcwNDE3NDA3MQ@@._V1_UY1200_CR115,0,630,1200_AL_.jpg",
        name: "Mark Fergus",
      },
      {
        photo:
          "https://static.cinemagia.ro/img/db/actor/02/34/34/hawk-ostby-628646l.jpg",
        name: "Hawk Ostby",
      },
    ],
    actors: [
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/9/94/Robert_Downey_Jr_2014_Comic_Con_%28cropped%29.jpg",
        name: "Robert Downey Jr.",
        role: "Tony Stark",
      },
      {
        photo:
          "https://fr.wikipedia.org/wiki/Chris_Evans_(acteur)#/media/Fichier:Mark_Kassen,_Tony_C%C3%A1rdenas_and_Chris_Evans_(cropped).jpg",
        name: "Chris Evans",
        role: "Capitain America",
      },
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/e/eb/Gwyneth_Paltrow_avp_Iron_Man_3_Paris_2.jpg",
        name: "Gwyneth Paltrow",
        role: "pepper potts",
      },
    ],
    plot: "Les plus grands héros de la Terre doivent s'unir et apprendre à se battre s'ils veulent empêcher Loki et son armée extraterrestre d'asservir l'humanité tout entière.",
    poster:
      "https://images.now-tv.com/shares/vod_images/vi_vodproduct_desc_t/201406/en_us/original_vertical/201406040092349_408_608",
    year: "2008",
    trailer: "tWCwPirJSck",
    reviews: "",
    streaming: [
      { name: "PrimeVideo", plataform: "https://www.primevideo.com" },
      { name: "Disney", plataform: "https://www.disneyplus.com" },
    ],
    stars: 5,
    favorite: true,
  },
  {
    id: "2",
    title: "Marvel's The Avengers",
    director: [
      {
        photo:
          "https://fr.wikipedia.org/wiki/Joss_Whedon#/media/Fichier:Joss_Whedon_by_Gage_Skidmore_8.jpg",
        name: "	Joss Whedon",
      },
    ],

    screenwriters: [
      {
        photo:
          "https://m.media-amazon.com/images/M/MV5BMTk1ODk2OTM0OF5BMl5BanBnXkFtZTcwNDE3NDA3MQ@@._V1_UY1200_CR115,0,630,1200_AL_.jpg",
        name: "Mark Fergus",
      },
      {
        photo:
          "https://static.cinemagia.ro/img/db/actor/02/34/34/hawk-ostby-628646l.jpg",
        name: "Hawk Ostby",
      },
    ],
    actors: [
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/9/94/Robert_Downey_Jr_2014_Comic_Con_%28cropped%29.jpg",
        name: "Robert Downey Jr.",
        role: "Tony Stark",
      },
      {
        photo:
          "https://fr.wikipedia.org/wiki/Chris_Evans_(acteur)#/media/Fichier:Mark_Kassen,_Tony_C%C3%A1rdenas_and_Chris_Evans_(cropped).jpg",
        name: "Chris Evans",
        role: "Capitain America",
      },
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/e/eb/Gwyneth_Paltrow_avp_Iron_Man_3_Paris_2.jpg",
        name: "Gwyneth Paltrow",
        role: "pepper potts",
      },
    ],
    plot: "Les plus grands héros de la Terre doivent s'unir et apprendre à se battre s'ils veulent empêcher Loki et son armée extraterrestre d'asservir l'humanité tout entière.",
    poster:
      "https://images.now-tv.com/shares/vod_images/vi_vodproduct_desc_t/201406/en_us/original_vertical/201406040092349_408_608",
    year: "2008",
    trailer: "tWCwPirJSck",
    reviews: "",
    streaming: [
      { name: "PrimeVideo", plataform: "https://www.primevideo.com" },
      { name: "Disney", plataform: "https://www.disneyplus.com" },
    ],
    stars: 2.5,
    favorite: true,
  },
  {
    id: "2",
    title: "Marvel's The Avengers",
    director: [
      {
        photo:
          "https://fr.wikipedia.org/wiki/Joss_Whedon#/media/Fichier:Joss_Whedon_by_Gage_Skidmore_8.jpg",
        name: "	Joss Whedon",
      },
    ],

    screenwriters: [
      {
        photo:
          "https://m.media-amazon.com/images/M/MV5BMTk1ODk2OTM0OF5BMl5BanBnXkFtZTcwNDE3NDA3MQ@@._V1_UY1200_CR115,0,630,1200_AL_.jpg",
        name: "Mark Fergus",
      },
      {
        photo:
          "https://static.cinemagia.ro/img/db/actor/02/34/34/hawk-ostby-628646l.jpg",
        name: "Hawk Ostby",
      },
    ],
    actors: [
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/9/94/Robert_Downey_Jr_2014_Comic_Con_%28cropped%29.jpg",
        name: "Robert Downey Jr.",
        role: "Tony Stark",
      },
      {
        photo:
          "https://fr.wikipedia.org/wiki/Chris_Evans_(acteur)#/media/Fichier:Mark_Kassen,_Tony_C%C3%A1rdenas_and_Chris_Evans_(cropped).jpg",
        name: "Chris Evans",
        role: "Capitain America",
      },
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/e/eb/Gwyneth_Paltrow_avp_Iron_Man_3_Paris_2.jpg",
        name: "Gwyneth Paltrow",
        role: "pepper potts",
      },
    ],
    plot: "Les plus grands héros de la Terre doivent s'unir et apprendre à se battre s'ils veulent empêcher Loki et son armée extraterrestre d'asservir l'humanité tout entière.",
    poster:
      "https://images.now-tv.com/shares/vod_images/vi_vodproduct_desc_t/201406/en_us/original_vertical/201406040092349_408_608",
    year: "2008",
    trailer: "tWCwPirJSck",
    reviews: "",
    streaming: [
      { name: "PrimeVideo", plataform: "https://www.primevideo.com" },
      { name: "Disney", plataform: "https://www.disneyplus.com" },
    ],
    stars: 1.5,
    favorite: true,
  },
  {
    id: "2",
    title: "Marvel's The Avengers",
    director: [
      {
        photo:
          "https://fr.wikipedia.org/wiki/Joss_Whedon#/media/Fichier:Joss_Whedon_by_Gage_Skidmore_8.jpg",
        name: "	Joss Whedon",
      },
    ],

    screenwriters: [
      {
        photo:
          "https://m.media-amazon.com/images/M/MV5BMTk1ODk2OTM0OF5BMl5BanBnXkFtZTcwNDE3NDA3MQ@@._V1_UY1200_CR115,0,630,1200_AL_.jpg",
        name: "Mark Fergus",
      },
      {
        photo:
          "https://static.cinemagia.ro/img/db/actor/02/34/34/hawk-ostby-628646l.jpg",
        name: "Hawk Ostby",
      },
    ],
    actors: [
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/9/94/Robert_Downey_Jr_2014_Comic_Con_%28cropped%29.jpg",
        name: "Robert Downey Jr.",
        role: "Tony Stark",
      },
      {
        photo:
          "https://fr.wikipedia.org/wiki/Chris_Evans_(acteur)#/media/Fichier:Mark_Kassen,_Tony_C%C3%A1rdenas_and_Chris_Evans_(cropped).jpg",
        name: "Chris Evans",
        role: "Capitain America",
      },
      {
        photo:
          "https://upload.wikimedia.org/wikipedia/commons/e/eb/Gwyneth_Paltrow_avp_Iron_Man_3_Paris_2.jpg",
        name: "Gwyneth Paltrow",
        role: "pepper potts",
      },
    ],
    plot: "Les plus grands héros de la Terre doivent s'unir et apprendre à se battre s'ils veulent empêcher Loki et son armée extraterrestre d'asservir l'humanité tout entière.",
    poster:
      "https://images.now-tv.com/shares/vod_images/vi_vodproduct_desc_t/201406/en_us/original_vertical/201406040092349_408_608",
    year: "2008",
    trailer: "tWCwPirJSck",
    reviews: "",
    streaming: [
      { name: "PrimeVideo", plataform: "https://www.primevideo.com" },
      { name: "Disney", plataform: "https://www.disneyplus.com" },
    ],
    stars: 5,
    favorite: true,
  },
];

export const sample_users: any[] = [
  {
    name: "Federico",
    email: "fede@test.fr",
    password: "1234",
    isAdmin: true,
  },
  {
    name: "Test",
    email: "test@test.fr",
    password: "123",
    isAdmin: true,
  },
];
